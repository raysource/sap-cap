...
/* 注意这里与 Node 版的差别（真实踩坑）：
   Node 运行时把 @path 当成完整路径，写 '/odata/v4/book-order' 即可。
   CAP Java 会把 servlet 的基础路径 '/odata/v4' 加到 @path 前面 —— 如果这里也写 '/odata/v4/book-order'，
   真实 URL 会变成 /odata/v4/odata/v4/book-order（404）。
   所以 Java 版写 '/book-order'，两边的**外部 URL 完全一致**：/odata/v4/book-order */
@path: '/book-order'
